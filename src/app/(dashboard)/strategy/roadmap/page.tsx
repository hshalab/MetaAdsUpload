...
import { Route, Plus, RefreshCw, X } from "lucide-react";
...
export default function RoadmapPage() {
  ...
  const [form, setForm] = useState({ conceptName: "", batchNumber: "", desireId: "", subAvatarId: "", angleId: "", awarenessLevel: "", fileType: "", hypothesis: "", status: "ideation" });

  const fetchData = useCallback(async () => {
    try {
      const [roadRes, avatarRes] = await Promise.all([
        fetch("/api/strategy/roadmap"),
  ...
  const handleCreate = async () => {
    ...
    if (res.ok) {
      setShowCreate(false);
      setForm({ conceptName: "", batchNumber: "", desireId: "", subAvatarId: "", angleId: "", awarenessLevel: "", fileType: "", hypothesis: "", status: "ideation" });
      fetchData();
    }
  };
            ...
            <div key={col.key} className={cn("rounded-xl border p-3 min-h-[300px]", col.color)}>
              ...
              <div className="space-y-2">
                {colEntries.map((entry) => (
                  ...
                  >
                    <div className="text-sm font-medium text-white truncate">{entry.conceptName}</div>
                    {entry.batchNumber && <div className="text-[10px] text-slate-500">Batch #{entry.batchNumber}</div>}
